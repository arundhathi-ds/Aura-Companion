...
const getCategoryGroup = (categoryKey: string): CategoryGroupKey | null => {
  ...
  const emotionalCategories = ["healing", "nostalgia", "overthinking-hours", "lonely-night", "quiet-curiosity"];
  const atmosphericCategories = ["rainy-window", "neon-city", "space-drift", "coffee-shop", "midnight-train", "ocean-waves"];
  ...
  const cinematicCategories = ["movie-ending", "main-character-walk", "cyberpunk-night", "slow-motion-memories"];
...
function TrackCard({ track, index, categoryColor }: { track: Track; index: number; categoryColor: string }) {
  return (
    <motion.a
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-[11px] text-muted-foreground">{track.artists[0] || "Unknown"}</p>
      </div>
...
export function CategoryBrowser() {
  ...
  const loadTracksForCategory = async (categoryKey: string) => {
    setLoading(true);
    try {
      const res = await recommend({ intent: categoryKey, weather: undefined, limit: 15 });
      const mappedTracks: Track[] = (res?.tracks ?? []).map((t: any) => ({
        id: t.id,
        name: t.name,
        artists: t.artists,
      ...
      MusicMemory.recordTracks(mappedTracks, categoryKey);
    } catch (e) {
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground">Emotional Soundtracks</p>
      ...
      <AnimatePresence mode="wait">
        {selectedCategoryProfile && (
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-semibold text-white">{selectedCategoryProfile.label}</h3>
