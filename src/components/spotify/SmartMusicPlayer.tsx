 ...
 * Supports English, Tamil, Hindi, Malayalam, Spanish, and Korean hits.
 */
...
export function SmartMusicPlayer() {
  ...
  const headline = intent && weather && language ? getVibeHeadline(intent, weather, language) : "";
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground">Global Harmonies</p>
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground">Discover Vibes</p>
        </div>
      ...
      <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {INTENT_OPTIONS.map((o) => (
                <OptionChip key={o.key} emoji={o.emoji} label={o.label} selected={intent === o.key} onClick={() => { setIntent(o.key); setStep("weather"); }} />
              ))}
            </div>
          </motion.div>
        )}
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {WEATHER_OPTIONS.map((o) => (
                <OptionChip key={o.key} emoji={o.emoji} label={o.label} selected={weather === o.key} onClick={() => { setWeather(o.key); setStep("language"); }} />
              ))}
            </div>
          </motion.div>
        )}
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {LANGUAGE_OPTIONS.map((o) => (
                <OptionChip key={o.key} label={o.label} selected={language === o.key} onClick={() => { setLanguage(o.key); generateResults(o.key); }} />
              ))}
            </div>
          </motion.div>
        )}
        ...
        {step === "results" && (
          <motion.div
            ...
            initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }}
          >
            <p className="mt-4 text-xs uppercase tracking-[0.2em] text-muted-foreground">{headline}</p>
            <ul className="mt-3 space-y-2">
              {songs.map((song, i) => <SongCard key={`${song.title}-${i}`} song={song} index={i} />)}
            </ul>
            <button onClick={reset} className="mt-4 text-xs text-muted-foreground underline-offset-4 hover:text-foreground hover:underline transition">
              Refresh Vibe & Shuffle →
            </button>
          </motion.div>
        )}
