 ...
 * Rotates playlist selections to prevent repetition.
 */
...
} from "@/lib/spotify/curated-playlists";
    ...
    >
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-[11px] text-muted-foreground">{playlist.trackCount} tracks</p>
        </div>
...
export function SpotifyMoodPlayer({ mood }: Props) {
  const playlists = useMemo(() => {
    const all = getPlaylistsForMood(mood);
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-[10px] text-muted-foreground/60">
        Opens in Spotify · rotates with your mood
      </p>
    </motion.div>
  );
}
